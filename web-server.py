...
while True:
    ...
    if  request == 'en' or request == 'index.html' or request == 'main_en' or request == '':
        ...
        with open(request, 'rb') as file:
            connection_socket.send("HTTP/1.1 200 OK\r\n".encode())
            connection_socket.send("Content-Type: image/svg+xml\r\n".encode())
            connection_socket.send("\r\n".encode())
            content = file.read()
            connection_socket.send(content)
    elif request == 'cr':
        ...
        connection_socket.send('Content-Type: text/html\r\n'.encode())
        connection_socket.send('Location: http://cornell.edu\r\n'.encode())
        connection_socket.send('\r\n'.encode())
    elif request == 'so':
        connection_socket.send('HTTP/1.1 307 Temporary Redirect\r\n'.encode())
        connection_socket.send('Content-Type: text/html\r\n'.encode())
        ...
        connection_socket.send('HTTP/1.1 307 Temporary Redirect\r\n'.encode())
        connection_socket.send('Content-Type: text/html\r\n'.encode())
        connection_socket.send('Location: http://ritaj.birzeit.edu'.encode())
        connection_socket.send('\r\n'.encode())
        ...
        html_content=f"""
         ...
         <p><strong>Student name : Omar Hussain  ID:1212739 </strong></p>
         <p><strong>Student name : Diaa Badaha  ID:1210478</strong></p>
         <p><strong>Student name : Ghazi Haj Qassem  ID:1210778 </strong></p>  
